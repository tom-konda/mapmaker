            ...
            for (let idx = 0; idx < cords.length; idx++) {
                if (cords[idx] !== undefined) {
                    ...
                    if (fcd[0] !== lcd[0] || fcd[1] !== lcd[1]) {       // no closed way(画面外側にCordを追加)
                        ...
                        if (fcdBbox.includes("N")) {        // fcdが北にある場合
                            if (!lcdBbox.includes("N")) {       // lcdが北に無い（南）場合、東が陸地
                                fcd_d += "E";
                            } else {
                        ...
                        if (fcdBbox.includes("S")) {        // fcdが南にある場合
                            if (!lcdBbox.includes("S")) {       // lcdが南に無い（北）場合、西が陸地
                                fcd_d += "W";
                            } else {
                        ...
                        if (fcdBbox.includes("W")) {        // fcdが西にある場合
                            if (!lcdBbox.includes("W")) {       // lcdが西に無い場合は北側が陸地
                                fcd_d += "N";
                            } else {
                                fcd_d += "S";
                            }
                        }
                        if (fcdBbox.includes("E")) {    // fcdが東にある場合
                            if (!lcdBbox.includes("E")) {           // lcdが東に無い場合は南側が陸地
                                fcd_d += "S";
                            } else {
                                fcd_d += "N";
                            }
                        ...
                        // lcd_設定
                        if (lcdBbox.includes("N")) {        // lcdが北にある場合
                            if (!fcdBbox.includes("N")) {       // fcdが北に無い（南）場合、西が陸地
                                lcd_d += "W";
                            } else {
                                lcd_d += lcd[0] > lpcd[0] ? "W" : "E";   // wayが西向きならW、他はE
                            }
                        }
                        if (lcdBbox.includes("S")) {        // lcdが南にある場合
                            if (!fcdBbox.includes("S")) {       // lcdが南に無い（北）場合、東が陸地
                                lcd_d += "E";
                            } else {
                                lcd_d += lcd[0] > lpcd[0] ? "W" : "E";   // wayが西向きならE、他はW
                            }
                        }
                        if (lcdBbox.includes("W")) {        // lcdが西にある場合
                            if (!fcdBbox.includes("W")) {       // fcdが西に無い場合は南側が陸地
                                lcd_d += "S";
                            } else {
                                lcd_d += "N";
                            }
                        }
                        if (lcdBbox.includes("E")) {        // lcdが東にある場合
                            if (!fcdBbox.includes("E")) {       // fcdが東に無い場合は北側が陸地
                                lcd_d += "N";
                            } else {
                                lcd_d += "S";
                            }
